    ...
    def _playout(self, board):
        ...
        if not end:
            ...
            if winner == -1:  # Tie
                ...
                leaf_value = (
                    1.0 if winner == board.get_current_player_id() else -1.0
                )
        # 在本次遍历中更新节点的值和访问次数
...
class MCTSPlayer(object):
    ...
    # 得到行动
    def get_action(self, board, temp=1e-3, return_prob=0, play_with_ucci=False):
        # 像alphaGo_Zero论文一样使用MCTS算法返回的pi向量
        move_probs = np.zeros(2086)
        board = copy.deepcopy(board)
        if CONFIG['use_ucci'] or play_with_ucci:
            acts = board.availables
            if len(acts) == 1:
                move = acts[0]
                move_probs[acts[0]] = 1
                return move, move_probs
            move, lines = get_ucci_move_func(board, board.current_player_color)
            if move is None:
                move = random.choice(board.availables)
                error_res = list(board.current_map())
                error_res.append(lines)
                error_log.error(str(error_res))
                print('ucci error')
            move_probs[move] = 1
            return move, move_probs
        else:
        ...
        if self._is_selfplay:
            ...
            move = np.random.choice(acts, p=0.75 * probs + 0.25 * np.random.dirichlet(
                CONFIG['dirichlet'] * np.ones(len(probs))))
            # 更新根节点并重用搜索树
            self.mcts.update_with_move(move)
        else:
            move = np.random.choice(acts, p=0.95 * probs + 0.05 * np.random.dirichlet(
                CONFIG['dirichlet'] * np.ones(len(probs))))
            # 重置根节点
            self.mcts.update_with_move(-1)
